document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (noticia) {
        noticiaContenido.innerHTML = `
            ...
            <img src="${noticia.imagen}" alt="${noticia.titulo}" class="noticia-imagen" loading="lazy">
        ...
        if (noticia.contenido) {
            noticia.contenido.forEach(bloque => {
                ...
                switch (bloque.tipo) {
                    case "titulo":
                        elemento = `<h3>${bloque.texto}</h3>`;
                        break;
                    case "texto":
                        elemento = `<p>${bloque.texto}</p>`;
                        break;
                    case "imagen":
                        elemento = `<img src="${bloque.url}" alt="${bloque.descripcion}" title="${bloque.descripcion}" class="noticia-imagen">`;
                        break;
                }
                noticiaContenido.innerHTML += elemento;
            });
        }
    } else {
